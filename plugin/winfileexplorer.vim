...
" recode current buffer's path
if !exists("g:cur_buf_path")
	let g:cur_buf_path = getcwd()
endif
...
function! s:EditDir(...)
	...
	if exists("b:displayMode") && b:displayMode == "winmanager"
		...
		nnoremap <buffer> C    :call <SID>EditDir(g:cur_buf_path,1)<cr>:call <SID>RestoreFileDisplay()<cr>
...
" Funtion for listing files of current path which editing file located in.
" History: broton
let g:old_buf_name=""
function! s:ListFileOfCurrentPath()
  ...
  let cur_full_name = expand("%:p")
  if filereadable(cur_full_name)
    let g:cur_buf_path = expand("%:p:h")
    let g:old_buf_name = cur_buf_name
	let @/ = ''
    exe 'FirstExplorerWindow'
    call s:EditDir(g:cur_buf_path,1)
    call s:RestoreFileDisplay()
	let @/ = ''
    exe 'GotoEditingWindow'
  endif
endfunction
